...
Adafruit_NeoPixel::~Adafruit_NeoPixel() {
  free(pixels);
  if(pin >= 0) pin_function((PinName)pin,STM_PIN_DEFINE(0b0000,0b00,0b0000));//pinMode(pin, INPUT);
}
...
void Adafruit_NeoPixel::begin(void) {
  if(pin >= 0) {
    pin_function((PinName)pin,STM_PIN_DEFINE(0x0001,0b00,0b0000));
...
#endif
#elif defined(ARDUINO_ARCH_STM32) || defined(ARDUINO_ARCH_ARDUINO_CORE_STM32)||defined(STM32F4xx)
  uint8_t           *p   = pixels, *end = p + numBytes,
                    pix = *p++, mask = 0x80;
  uint32_t          cyc;
  uint32_t saveLoad = SysTick->LOAD, saveVal = SysTick->VAL,saveControl=SysTick->CTRL,
            saveGPIOCCLK=  __HAL_RCC_GPIOC_IS_CLK_ENABLED();
GPIO_InitTypeDef GPIO_DeInit={
  ...
  if(is800KHz) {
#endif
LL_RCC_ClocksTypeDef CLKINFO;
LL_RCC_GetSystemClocksFreq(&CLKINFO);
 //uint32_t freq_temp= CLKINFO.PCLK1_Frequency;//HAL_RCC_GetSysClockFreq();
 uint32_t freq_temp= CLKINFO.SYSCLK_Frequency;
 __HAL_RCC_GPIOC_CLK_ENABLE();
     pin_function((PinName)pin,STM_PIN_DEFINE(0x0001,0b00,0b0000));
    LL_GPIO_ResetOutputPin(gpioPort, gpioPin);
    uint32_t top = (freq_temp  /  800000);       // 1.25µs
    uint32_t t0  = top - (freq_temp / 2500000); // 0.4µs
    uint32_t t1  = top - (freq_temp / 1250000); // 0.8µs
    SysTick->CTRL&=~SysTick_CTRL_ENABLE_Msk;
    SysTick->CTRL|=SYSTICK_CLKSOURCE_HCLK&(~SysTick_CTRL_TICKINT_Msk);
  ...
  SysTick->LOAD = saveLoad;          // Restore SysTick rollover to 1 ms
  SysTick->VAL  = saveVal;           // Restore SysTick value
  SysTick->CTRL=saveControl;
  HAL_GPIO_Init(gpioPort,&GPIO_DeInit);
  if(!saveGPIOCCLK){
...
void Adafruit_NeoPixel::setPin(uint16_t p) {
  if(begun && (pin >= 0)) pin_function((PinName)pin,STM_PIN_DEFINE(0b0000,0b00,0b0000));//pin_mode(pin, INPUT);
  pin = p;
  if(begun) {
    pin_function((PinName)p,STM_PIN_DEFINE(0x0001,0b00,0b0000));
    LL_GPIO_ResetOutputPin(Set_GPIO_Clock(STM_PORT(p)), ll_pin_defines[STM_PIN(p)]);
    //pinMode(p, OUTPUT);
    //digitalWrite(p, LOW);
  }
#if defined(__AVR__)
...
#endif
#if defined(ARDUINO_ARCH_STM32) || defined(ARDUINO_ARCH_ARDUINO_CORE_STM32)||defined(STM32F4xx)
  gpioPort = Set_GPIO_Clock(STM_PORT(p));
  gpioPin =ll_pin_defines[STM_PIN(p)];
