 ...
 uint8_t SetSysClock_PLL_HSE(uint8_t bypass)
{
    ...
    while((*((uint32_t*)(0x40007000U + 0x04U)) & (~0x8U)) <= 0){
    }

    
    //high speed mode 168Mhz
    __HAL_PWR_VOLTAGESCALING_CONFIG(PWR_REGULATOR_VOLTAGE_SCALE1);
    
    
    RCC_OscInitStruct.PLL.PLLSource         = RCC_PLLSOURCE_HSE; // 12 MHz
    ...
    RCC_ClkInitStruct.APB1CLKDivider = RCC_HCLK_DIV4;           // 80 MHz or 48 MHz
    RCC_ClkInitStruct.APB2CLKDivider = RCC_HCLK_DIV2;           // 80 MHz or 48 MHz
